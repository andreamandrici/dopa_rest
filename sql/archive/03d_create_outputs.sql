...
CREATE TABLE mtz_results.country_ecoregions_stats_template AS SELECT json_array_elements(ecoregion_in_country) ecoregion_in_country FROM mtz_results.country_all_inds LIMIT 0;
-- CREATE FUNCTION get_country_ecoregions_stats
CREATE FUNCTION mtz_results.get_country_ecoregions_stats()
RETURNS SETOF mtz_results.country_ecoregions_stats_template LANGUAGE SQL AS $BODY$ SELECT json_array_elements(ecoregion_in_country) ecoregion_in_country FROM mtz_results.country_all_inds; $BODY$;
