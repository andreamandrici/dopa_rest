...
LEFT JOIN (
		SELECT country_id,ARRAY_AGG((b.*)) ecoregion_in_country
		FROM mtz_results.ecoregion_in_country b
...
CREATE TABLE mtz_results.country_ecoregions_stats_template AS SELECT (UNNEST(ecoregion_in_country)).* FROM mtz_results.country_all_inds LIMIT 0;
-- CREATE FUNCTION get_country_ecoregions_stats
CREATE FUNCTION mtz_results.get_country_ecoregion_stats()
RETURNS SETOF mtz_results.country_ecoregions_stats_template LANGUAGE SQL AS $BODY$ SELECT (UNNEST(ecoregion_in_country)).* FROM mtz_results.country_all_inds; $BODY$;
